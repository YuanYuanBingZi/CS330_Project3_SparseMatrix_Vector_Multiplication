...
{       
    unsigned int* csr_row_ptr_ = (unsigned int*)malloc(sizeof(unsigned int)*(nnz+1));
    unsigned int* csr_col_ind_ = (unsigned int*)malloc(sizeof(unsigned int)*nnz);
    double* csr_vals_ = (double *) malloc(sizeof(double)* nnz);
    for (int i = 0; i < nnz; i++){
	csr_row_ptr_[i] = row_ind[i];
	csr_col_ind_[i] = col_ind[i];    
        csr_vals_[i] = val[i];

    }
  *csr_row_ptr= csr_row_ptr_;
  *csr_vals = csr_vals_;
  *csr_col_ind = csr_col_ind_;
  }
          ...
          double* vector_x, double* res)
{
  for(int i = 0; i < m; i++){
	  res[i] = 0;}
  for(int i = 0; i < nnz; i++){
	  res[csr_row_ptr[i]-1] += csr_vals[i] * vector_x[csr_col_ind[i]-1];}
}
